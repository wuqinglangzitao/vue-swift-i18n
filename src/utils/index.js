...
const getLocales = ({
    ...
    if (fs.existsSync(path.join(dirName, pkgFileName))) {
        ...
        if (path.extname(fsPath).includes(".")) {
            const fileName = path.extname(fsPath);

            const name = fsPath.slice(0, fsPath.indexOf(".")).split(path.sep);
            filePath = [name[name.length - 1], ...filePath];
        } else {
            const name = fsPath.split(path.sep);
            filePath = [name[name.length - 1], ...filePath];
        }
        return getLocales({
            fsPath: dirName,
